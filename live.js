...
let _liveAbortCtrl = null;            // AbortController for current player request
...
function _liveRender(snap, baseline) {
  ...
  root.querySelectorAll("[data-live-player]").forEach(b => {
    b.addEventListener("click", () => {
      ...
      if (_liveAbortCtrl) _liveAbortCtrl.abort();  // Abort any in-flight request for previous player
      _livePlayerIdx = idx;
      _liveSamples = [];
      _liveLerpPrev = null;
      _liveLerpRate = null;
